...
> 来源：[零到全栈 · 模块 5.4](https://xn--ygr25xpohxwz.com/zero-to-fullstack/lessons/module-5-4/)  
> 课程：零到全栈 · API 与后端  
> 日期：2026 年 07 月 23 日
...
- [那些杂活，每个后端都一样](#那些杂活每个后端都一样)
- [认识几个主流后端框架](#认识几个主流后端框架)
- [两个角色：FastAPI 和 uvicorn](#两个角色fastapi-和-uvicorn)
- [把 FastAPI 装进来](#把-fastapi-装进来)
- [用 FastAPI 重写 `/api/profile`](#用-fastapi-重写-apiprofile)
- [跑起来：先用 uvicorn「手动挡」](#跑起来先用-uvicorn手动挡)
- [换官网的快捷命令：`fastapi dev`](#换官网的快捷命令fastapi-dev)
  - [目录里多了一个东西](#目录里多了一个东西)
  - [惊喜：我们的 API 自己长出了文档](#惊喜我们的-api-自己长出了文档)
- [加一个 POST 接口：`/api/analyze`](#加一个-post-接口apianalyze)
  - [先把需求说清楚：一来一回长什么样](#先把需求说清楚一来一回长什么样)
  - [动手：三步写完](#动手三步写完)
  - [回头看：这三步在干嘛](#回头看这三步在干嘛)
- [最后一件事：报错了怎么看](#最后一件事报错了怎么看)
- [收尾：把依赖清单更新好](#收尾把依赖清单更新好)
- [这一节应该带走什么](#这一节应该带走什么)
...
- 上一节：[模块 5.3：看懂 HTTP，手搓 API](https://xn--ygr25xpohxwz.com/zero-to-fullstack/lessons/module-5-3/)
- 下一节：模块 5.5 前后端联调与 CORS
- FastAPI 官方教程：https://fastapi.tiangolo.com/tutorial/
- FastAPI 框架对比（About）：https://fastapi.tiangolo.com/alternatives/

**标签：** FastAPI · Uvicorn · API · 后端 · Python · Pydantic · 框架 · 全栈
